...
def add_student(student_id, name, age, *grades):
    ...
    # store immutable info (student_id, name) as a tuple
    info = (student_id, name)

    # store all data in dictionary, student_id as key
    students[student_id] = {
        "info": info,
        "age": age,
        "grades": list(grades)  # turn grades into a list (tuple to list)
    }
...
def view_students(from_file=False):
    ...
    # Loop through students dictionary using items() to get key-value pairs
